...
def main_v1():
    ...
    for llm_name, llm_ratings_filename in zip(llm_names, llm_ratings_paths):
        ...
        llm_ratings['video_path'] = llm_ratings['Video'].apply(lambda x: f'/localdisk1/PARK/park_vlm/Videos/{x}').astype(str)
        llm_ratings['question_id'] = llm_ratings['Question_ID']
        llm_ratings.drop(columns=['Question_ID'])
        ...
        results_path = os.path.join(results_base_path, f"cov_gpt5_automated_scores_overall_{llm_name}.json")
    ...
    for llm_name, llm_ratings_filename in zip(llm_names, llm_ratings_paths):
        ...
        for i, r in combined_ratings.iterrows():
            ...
            if llm_response is None:
                llm_response = ""
            bleu, R1, RL = compute_scores(hypothesis=llm_response, ref1=r['answer1'], ref2=r['answer2'], ref3=r['answer3'])
            bleu_scores.append(bleu)
            R1_scores.append(R1)
            RL_scores.append(RL)
        ...
        results_base_path = "/localdisk1/PARK/park_vlm_finetuning/results"
        os.makedirs(results_base_path, exist_ok=True)
        results_path = os.path.join(results_base_path, f"cov_llava_automated_scores_overall_{llm_name}.json")
        with open(results_path, "w") as f:
            json.dump(final_results, f, indent=4)

        print(f"Saved results for {llm_name} to {results_path}")
