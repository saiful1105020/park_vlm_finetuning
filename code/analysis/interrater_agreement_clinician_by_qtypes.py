...
def main():
    ...
    for qtype_id in range(4):
        ...
        print(f"QTYEPE: {qtype}, Count: {len(ratings_subset)}")

        agreement_metrics = AgreementMetrics(ratings_subset, 3)
        metrics = agreement_metrics.compute_agreement_metrics()
        results_base_path = "/localdisk1/PARK/park_vlm_finetuning/results"
        os.makedirs(results_base_path, exist_ok=True)
        results_path = os.path.join(results_base_path, f"clinician_rater_agreement_{''.join(qtype.split())}.json")
