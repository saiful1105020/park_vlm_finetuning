...
def main_v1():
    ...
    for llm_name, llm_ratings_filename in zip(llm_names, llm_ratings_paths):
        ...
        results_path = os.path.join(results_base_path, f"cov_gpt5_performance_overall_{llm_name}.json")
...
def main_v2():
    ...
    for llm_name, llm_ratings_filename in zip(llm_names, llm_ratings_paths):
        ...
        results_base_path = "/localdisk1/PARK/park_vlm_finetuning/results"
        os.makedirs(results_base_path, exist_ok=True)
        results_path = os.path.join(results_base_path, f"cov_llava_performance_overall_{llm_name}.json")
        with open(results_path, "w") as f:
            json.dump(metrics, f, indent=4)
