...
def map_response_to_likert(row, a_id=None):
    """
    Maps an open-ended response to a Likert score (1-5).
    # column names #
    video_path,question,answer,question_id,annotator
    """
    qid = row['question_id']
    ...
    if a_id is None:
        ...
        llm_response = row[f'answer{a_id}']
...
#     # )
#     # output = completion.choices[0].message.content
#     output = "This is a dummy response. Likert Score: 3"  # DUMMY
#     match = re.search(r"Likert Score: (\d)", output)
#     likert_score = int(match.group(1)) if match else None
#     return (likert_score, output.strip())

def process_clinician_responses(input_csv, output_csv):
    """
    ...
    if fold == "Train":
        ...
        # video_path,question,question_id,answer1,annotator1,answer2,annotator2,answer3,annotator3
        df["positive_statement"] = None
        for a_id in range(1,4):
            df[f"likert{a_id}_score"] = None
            df[f"likert{a_id}_explanation"] = None

            for i, r in tqdm(df.iterrows()):
                likert_score, explanation = map_response_to_likert(r, a_id)
                df.at[i, f"likert{a_id}_score"] = likert_score
                df.at[i, f"likert{a_id}_explanation"] = explanation
                if a_id==1:
                    qid = r['question_id']
                    new_question_text = positive_statements[qid]
                    df.at[i, "positive_statement"] = new_question_text
            ...
            df.to_csv(output_csv, index=False)
            print(f"Liker conversion done for annotator {a_id}")
...
if __name__ == "__main__":
    INPUT_FILES = {
        "train": "/localdisk1/PARK/park_vlm_finetuning/data/QA_dataset/combined_qa_dataset_train.csv",
        "test": "/localdisk1/PARK/park_vlm_finetuning/data/QA_dataset/combined_qa_dataset_test.csv"
    }

    OUTPUT_FILES = {
        "train": "/localdisk1/PARK/park_vlm_finetuning/data/QA_dataset/combined_qa_dataset_train_likert.csv",
        "test": "/localdisk1/PARK/park_vlm_finetuning/data/QA_dataset/combined_qa_dataset_test_likert.csv"
    }

    for fold in ["test", "train"]:
        input_csv = INPUT_FILES[fold]
        output_csv = OUTPUT_FILES[fold]
        print(f"Processing {input_csv}...")
        process_clinician_responses(input_csv, output_csv)
